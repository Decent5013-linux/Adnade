...
class SimpleBotSimulator:
    ...
    async def create_scroll_space(self):
        ...
        await self.page.evaluate(f"""
            ...
            // Add spacer to reach target content height
            const neededHeight = Math.max(0, {target_content} - currentHeight + {viewport_height});
            ...
            if (neededHeight > 100) {{
                ...
                console.log(`Added {neededHeight}px scroll spacer for {target_content}px content`);
            }}
        """)
    ...
    async def human_scroll(self, current_position=None, target_position=None):
        ...
        if target_position is None:
            ...
            scroll_amount = target_position - (current_position or 0)
        ...
        for i in range(steps):
            clicks_this_step = max(1, int(step_size / 40))  # ~40px per click
    ...
    async def run_session(self):
        ...
        # Create realistic scroll space
        content_height = await self.create_scroll_space()
        
        # Wait for ad to load
        await asyncio.sleep(2)
        
        # Calculate realistic session duration
        # Base reading time: 45-65 pixels per second
        min_read_speed = 45
        max_read_speed = 65
        ...
        # Add extra time for:
        # - Initial orientation: 2-4 sec
        # - Mouse movements/hovering: 4-8 sec
        # - Pauses between actions: 5-10 sec
        # - Decision time: 2-5 sec
