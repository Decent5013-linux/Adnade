...
async def get_proxy_credentials():
    """Fetch proxy username and password from secret URL and ALWAYS generate new session"""
    # Always generate new session - no reuse
...
async def mobile_interact(page, viewport, ua):
    """100% interaction guaranteed - Always scroll and interact with iframe"""
    
    # --- Step 1: Real touch scroll (ALWAYS) ---
    ...
    # --- Step 2: Always interact with iframe ---
    iframe_el = await page.query_selector("iframe")
    if iframe_el:
        box = await iframe_el.bounding_box()
        if box:
            # First touch on iframe (ALWAYS)
            ...
            # --- Step 3: Always attempt second tap (100% chance now) ---
            click_chance = 1.0  # Always click
...
async def desktop_interact(page, viewport):
    """100% interaction guaranteed - Always hover/focus and click"""
